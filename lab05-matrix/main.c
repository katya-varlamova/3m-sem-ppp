...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        MPI_Type_vector(10, 1, 10, MPI_INT, &col); // 10 blocks with 10 stride, 1 int is reserved from 10
        ...
        MPI_Type_struct(2, len, adr, tp, &col1); // blocks, length of each block, displacement, types, where 
        MPI_Type_commit(&col1);
        MPI_Type_free(&col);
        char buf_rec[1000];
        sprintf(buf_rec, "Receive %s[%d] %d of %d\n", name, nlen, myrank, nprocs);
        if (myrank % 4 == 1)
        ...
        {
            for (int j = 0; j < 10; j++)
                sprintf(buf_rec + strlen(buf_rec), "%6d", a[i][j]);
            sprintf(buf_rec  + strlen(buf_rec), "\n");
        }
